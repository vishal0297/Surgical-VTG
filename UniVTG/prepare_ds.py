...
from run_on_video.clip_feature_extractor import extractor
from main.dataset import DatasetMR, start_end_collate_mr
...
def get_duration(vid):
    path = "../labeled_surgical_videos/video/"+ str(vid)+".mp4"
...
def create_dataset():
    ...
    f = open("data/mr_data.jsonl")
    data = json.load(f)
    mr_surgvids = []
    for video in data['videos']:
        for keystep in video["keysteps"]:
            surg_vid = {}
            surg_vid["query"]=keystep["description"]
            surg_vid["qid"] = str(uuid.uuid4())
            surg_vid["vid"] = video["video_id"]
            surg_vid["relevant_windows"] = [[keystep["start_time"]/60,keystep["end_time"]/60]]
            surg_vid["duration"] = int(get_duration(surg_vid["vid"]))
            mr_surgvids.append(surg_vid)

    with open("data/mr_surg_vids.jsonl", 'w') as f:
        for item in mr_surgvids:
            f.write(json.dumps(item) + "\n")

def create_feature_dir():
    with open("data/mr_surg_vids.jsonl") as json_file:
        data = list(json_file)
    for i,d_str in enumerate(data):
        d = json.loads(d_str)
        vid_path = "/home/csgrad/vrajasek/surgical_videos_vlm/labeled_surgical_videos/video/"+ str(d['vid'])+".mp4"
        query = d["query"]
        extractor(vid_path, [query], "",d["qid"])
...
def compute_mr_results(model, eval_loader, opt, epoch_i=None, criterion=None, tb_writer=None):
    ...
    post_processor = PostProcessorDETR(
        clip_length=1.0, min_ts_val=0, max_ts_val=150,
...
def eval_epoch_post_processing(submission, opt, gt_data, save_submission_filename):
    submission_path = os.path.join("results", save_submission_filename)
...
if __name__ =="__main__":
    ...
    eval_dataset = DatasetMR(
            dset_name="vlp",
            data_path=["data/mr_surg_vids.jsonl"],
            v_feat_dirs=[
            "/home/csgrad/vrajasek/surgical_videos_vlm/UniVTG/vid_clip"
        ],
            q_feat_dir="/home/csgrad/vrajasek/surgical_videos_vlm/UniVTG/txt_clip",
            v_feat_dim=512,
            q_feat_dim=512,
    ...
    save_submission_filename = "inference_{}_{}_{}_preds.jsonl".format(
        opt.dset_name, opt.eval_split_name, 1)
    submission, eval_loss_meters=compute_mr_results(vtg_model,eval_loader,opt)
